...
const getAllSubtasks = async(req, res) => {
    ...
    if (result.length === 0)
        res.status(204).json({ message: "No mainTasks found" }); //no content
...
const createNewSubtask = async(req, res) => {
    ...
    if (!req ? .body ? .maintask_id ||
        !req ? .body ? .name ||
        !req ? .body ? .assigned_employees ||
        !req ? .body ? .note
    ) {
        return res.status(400).json({
            message: "maintask_id, name, note and assigned_employees of the new subTask are required.",
        }); //bad request
    }
...
const updateSubtask = async(req, res) => {
    ...
    if (!subTask) {
        return res
            .status(400)
            .json({ message: `Subtasks with id = ${id} is not found` }); //bad request
    }

    if (req.body.maintask_id) subTask.maintask_id = req.body.maintask_id;
    if (req.body.name) subTask.name = req.body.name;
    if (req.body.assigned_employees)
        subTask.assigned_employees = req.body.assigned_employees;
...
const deleteSubtask = async(req, res) => {
    ...
    if (!subTask) {
        return res
            .status(400)
            .json({ message: `Subtasks with ID ${req.body.id} not found` }); //bad request
    }
...
const getSubtask = async(req, res) => {
    ...
    if (!subTask) {
        return res
            .status(400)
            .json({ message: `Subtasks ID with ${req.body.id} not found` }); //bad request
    }
...
const getAllSubtasksOfMaintask = async(req, res) => {
    ...
    if (!allSubtasks) {
        return res.status(400).json({
            message: `Subtasks with maintasksID=${req.params.id} not found`,
        }); //bad request
    }
